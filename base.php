...
$id_especie = $_GET['id'] ?? false; // Obtener el ID correctamente  
...
// Verifica si se pasó un ID de especie  
if ($id_especie) {  
    $personajes = $miObjetoPersonajes->catalogo_x_especie($id_especie);  
    // Verificar si se obtuvieron resultados  
    if (empty($personajes)) {  
        $mensaje = "No se encontraron personajes para esta especie.";  
    }  
} else {  
    $mensaje = "ID de especie no válido.";  
}  

// Obtener la especie correspondiente si existe  
...
<body>
  ...
  <div class="grid-contenedor">
    ...
    <?php if (!empty($catalogo)): ?>  
        <?php foreach ($catalogo as $personaje): ?>  

            <div class="card"> 
                <h2 class="vermastitulo"><?php echo htmlspecialchars($personaje->getNombre()); ?></h2>   
                <div class="imgContainer" style="--imgUrl: url(../img/<?php echo htmlspecialchars($personaje->getImagenUno()); ?>)"></div> 
                <div class="img" style="--pngImgUrl: url(../img/<?php echo htmlspecialchars($personaje->getImagenDos()); ?>)">  
                    <div class="overLayer"></div>  
                </div>  
                <div class="vermas">  
                    <h3><a href="index.php?sec=personajes&id=<?php echo htmlspecialchars($personaje->getId()); ?>" target="_self">Ver más</a></h3>
                </div>  
            </div>  
            <?php endforeach; ?>  
    <?php else: ?>  
