<?php
require_once '../classes/Sables.php';
require_once '../classes/Nombre_Sables.php';

$id_nombre_sables= $_GET['id'] ?? false; // Cambiado para obtener el ID correctamente  

$miObjetoSables = new Sables();
$sables = $miObjetoSables->catalogo_x_nombre_sables($id_nombre_sables);

$nombre_sables = (new Nombre_Sables())->get_x_id($id_nombre_sables);
?>



<!DOCTYPE html>
<html lang="es">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Sables de Luz</title>



    <!-- vincular estilos -->
    <link rel="stylesheet" href="../css/estilos.css">
    <link rel="stylesheet" href="../css/sables.css">

    <!-- Animated -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css">

    <!-- FontAwesome CDN -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css"
        integrity="sha512-DTOQO9RWCH3ppGqcWaEA1BIZOC6xxalwEsw9c2QQeAIftl+Vegovlnee1c9QX4TctnWMn13TZye+giMm8e2LwA=="
        crossorigin="anonymous" referrerpolicy="no-referrer">
    <!-- Favicon-->
    <link rel="icon" href="../img/ico/icono.ico">

    <!-- font -->
    <link href="https://fonts.cdnfonts.com/css/starjedi-special-edition" rel="stylesheet">



</head>

<body>

    <div class="grid-contenedor">
        <div class="articulo1">

        </div>
        <div class="articulo2">

            <nav>

                <ul class="nav">


                    <li><a href="inicio.php" target="_self">inicio</a></li>
                    <li><a href="personajes.php" target="_self">Personajes</a></li>
                    <li><a href="naves.php" target="_self">Naves</a></li>
                    <li><a href="sables.php" target="_self">Sables de Luz</a></li>
                    <li><a href="peliculas.php" target="_self">Peliculas</a></li>
                    <li><a href="../index.php" target="_self">Presentación</a></li>
                    <li><a href="creditos.php" target="_self">Créditos</a></li>


                </ul>
            </nav>
        </div>
        <div class="articulo3">
            <aside>

            </aside>
        </div>

        <div class="articulo4">
            <header>
                <h1>Sables de Luz</h1>
            </header>
        </div>
        <div class="articulo5">
            <main>
                
                <p>En el universo de Star Wars, los sables de luz son icónicos, pero las frases que se centran
                    específicamente en ellos son menos comunes. Sin embargo, hay diálogos memorables que involucran
                    sables de luz, especialmente en momentos de combate o reflexiones sobre su significado. Por ejemplo,
                    una famosa frase de Obi-Wan Kenobi es: "Una antigua arma de un tiempo más civilizado", refiriéndose
                    al sable de luz como símbolo de la nobleza de los Jedi.
                </p>
                <br>

                <h2>Tipos de Sables de Luz</h2>


                <div class="lightsabers-container">
                    <div class="lightsaber" id="saber-red">
                        <div class="blade" id="blade-red"></div>
                        <div class="hilt"></div>
                        <button class="toggleButton" data-target="blade-red">Encender/Apagar</button>
                        <h3>Luz Roja</h3>
                        <a class="link-info" href="#roja">
                            <h3>Información</h3>
                        </a>
                    </div>
                    <div class="lightsaber" id="saber-blue">
                        <div class="blade" id="blade-blue"></div>
                        <div class="hilt"></div>
                        <button class="toggleButton" data-target="blade-blue">Encender/Apagar</button>
                        <h3>Luz Azul</h3>
                        <a class="link-info" href="#azul">
                            <h3>Información</h3>
                        </a>
                    </div>
                    <div class="lightsaber" id="saber-green">
                        <div class="blade" id="blade-green"></div>
                        <div class="hilt"></div>
                        <button class="toggleButton" data-target="blade-green">Encender/Apagar</button>
                        <h3>Luz Verde</h3>
                        <a class="link-info" href="#verde">
                            <h3>Información</h3>
                        </a>
                    </div>
                    <div class="lightsaber" id="saber-purple">
                        <div class="blade" id="blade-purple"></div>
                        <div class="hilt"></div>
                        <button class="toggleButton" data-target="blade-purple">Encender/Apagar</button>
                        <h3>Luz Morada</h3>
                        <a class="link-info" href="#morada">
                            <h3>Información</h3>
                        </a>
                    </div>
                    <div class="lightsaber" id="saber-yellow">
                        <div class="blade" id="blade-yellow"></div>
                        <div class="hilt"></div>
                        <button class="toggleButton" data-target="blade-yellow">Encender/Apagar</button>
                        <h3>Luz Amarilla</h3>
                        <a class="link-info" href="#amarilla">
                            <h3>Información</h3>
                        </a>
                    </div>
                    <div class="lightsaber" id="saber-black">
                        <div class="blade" id="blade-black"></div>
                        <div class="hilt"></div>
                        <button class="toggleButton" data-target="blade-black">Encender/Apagar</button>
                        <h3>Luz Negra</h3>
                        <a class="link-info" href="#negro">
                            <h3>Información</h3>
                        </a>
                    </div>
                </div>
                <br>

                <h2 id="roja">Sable de Luz Roja</h2>


                <p class="republica2">Poder, pasión, ira, odio y sufrimiento. Usado en su mayoría por los Señores
                    Oscuros Sith. Al principio existían cristales Kyber de color rojo naturales pero con el tiempo y su
                    uso constante por parte de los Sith, estos se fueron perdiendo hasta que prácticamente ya no quedaba
                    ninguno, sin embargo con el tiempo los Sith encontraron una forma de crear un cristal rojo
                    artificial utilizando cualquier otro color de cristal mediante el uso del poder del Lado Oscuro.</p>

                <p class="republica2">Como en cada tradición luego de la extinción de los cristales Kyber rojos, cada
                    aspirante a lord Sith debía asesinar a un Jedi y despojarlo de su sable de luz, después, mediante el
                    uso de la fuerza el cristal es corrompido y el cristal se alimenta del sufrimiento del usuario (esto
                    según los cómics de Darth Vader) de ahí se podrá obtener este característico color de cristal, sin
                    embargo hay que tener en cuenta que si la concentración del usuario oscuro no es absoluta o en otros
                    casos si en pleno proceso el cristal se fragmenta este al momento de colocarlo en su sable de luz
                    del usuario se volverá muy inestable y puede correr el riesgo de que su sable de luz se
                    sobrecaliente y se destruya.</p>


                <div class="galeriasable">
                    <div class="fotosable"><a href="../img/sable/darth.webp" target="_blank"><img class="fotosable-1"
                                src="../img/sable/darth.webp" alt="darth"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/dooku.webp" target="_blank"><img class="fotosable-1"
                                src="../img/sable/dooku.webp" alt="dooku"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/kylo.webp" target="_blank"><img class="fotosable-1"
                                src="../img/sable/kylo.webp" alt="kylo"></a>
                    </div>
                </div>

                <p class="republica2"> Como en el caso del sable de luz de plasma de Kylo Ren, ya que este último
                    tuvo que hacer una serie de modificaciones a su sable de luz original, colocando una serie de
                    ventiladores en los dos costados de su empuñadura para de esta forma estabilizar su cristal
                    fragmentado a una forma más segura sin que la empuñadura se destruya. Casi todos los Sith en la
                    historia usaban este color como lo son: Darth Bane, Darth Revan, Darth Sidious, Darth Maul,
                    Conde Dooku, Asajj Ventress, Savage Opress, Darth Vader, todos los Inquisidores y Kylo Ren.</p>

                <p class="republica2"><Strong>Empuñadura:</Strong>
                    Las empuñaduras de los sables rojos pueden variar desde formas elegantes hasta diseños más
                    intimidantes y agresivos.
                    Por ejemplo, el sable de Darth Vader tiene una empuñadura más gótica, mientras que la de Darth Maul
                    se considera muy distintiva por su doble hoja.
                    <br>
                    <Strong>Cristal:</Strong>
                    Los cristales rojos son artificiales o "sangrados", ya que los Sith suelen manipular los cristales
                    kyber naturales para que expresen su propia ira y rencor.
                    Este proceso les da un brillo rojo intenso simbolizando el dominio sobre la Fuerza.
                    <br>
                    <Strong>Hoja:</Strong>
                    La hoja roja es feroz y se asocia con la agresividad y el deseo de poder.
                    A menudo se representa como una presencia amenazante y oscura.
                </p>

                <!-- Inicio de Pagina -->
                <div class="botones-fin2">
                    <div class="icono-container">

                        <a href="#"><svg xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 512 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                                <path fill="#FFFFFF"
                                    d="M249.9 33.5v26.1C236.3 80.2 226 168.1 225.4 274.9c11.7-15.6 19.1-33.3 19.1-48.2v-16.9c0-5.3 .8-10.5 2.2-15.7 .7-2.1 1.4-4.1 2.6-5.8 1.2-1.8 3.4-3.8 6.7-3.8 3.2 0 5.5 2.1 6.7 3.8 1.2 1.8 2 3.7 2.6 5.8 1.4 5.1 2.2 10.3 2.2 15.7v16.9c0 14.9 7.4 32.6 19.1 48.2-.6-106.8-10.9-194.7-24.5-215.4V33.5h-12.3zm-26.3 147.8c-9.5 2.2-18.7 5.2-27.5 9.1 8.9 16.1 9.8 32.6 1.7 37.3-8 4.6-21.9-4.2-31.4-19.8-11.6 8.8-21.9 19.3-30.6 31.1 14.7 9.6 22.9 22.9 18.3 30.7-4.5 7.7-20 7.1-35.5-1-5.8 13.3-9.8 27.5-11.7 42.4 9.7 .2 18.7 2.4 26.2 6 17.8-.3 32.8-2 40.5-4.2 5.6-26.4 23-48.2 46.3-59.5 .7-25.6 1.9-49.7 3.5-72.1zm65 0c1.6 22.4 2.8 46.5 3.5 72.1 23.3 11.3 40.8 33.2 46.3 59.5 7.7 2.3 22.7 3.9 40.5 4.2 7.5-3.7 16.5-5.9 26.2-6-1.9-14.9-5.9-29.2-11.7-42.4-15.4 8.1-30.9 8.7-35.5 1-4.6-7.7 3.6-21.1 18.3-30.7-8.7-11.8-19-22.3-30.6-31.1-9.5 15.6-23.4 24.4-31.4 19.8-8.1-4.7-7.2-21.2 1.7-37.3a147.5 147.5 0 0 0 -27.5-9.1zm-32.5 8.6c-3.2 0-5.9 8.8-6.1 19.9h-.1v16.9c0 41.4-49 95-93.5 95-52 0-122.8-1.5-156.4 29.2v2.5c9.4 17.1 20.6 33.2 33.2 48C45.7 380.3 84.8 360.4 141.2 360c45.7 1 79 20.3 90.8 40.9 0 0 0 0 0 .1 7.7 2.1 15.9 3.2 24 3.2 8.2 0 16.4-1.1 24-3.2 0 0 0 0 0-.1 11.7-20.5 45.1-39.9 90.8-40.9 56.4 .4 95.5 20.3 108 41.4 12.6-14.8 23.8-30.9 33.2-48v-2.5c-33.6-30.6-104.4-29.2-156.4-29.2-44.5 0-93.5-53.6-93.5-95v-16.9h-.1c-.2-11.1-2.9-19.9-6.1-19.9zm0 96.6c22.4 0 40.6 18.2 40.6 40.6s-18.2 40.7-40.6 40.7-40.6-18.2-40.6-40.7c0-22.4 18.2-40.6 40.6-40.6zm0 7.6c-18.2 0-33 14.8-33 33S237.8 360 256 360s33-14.8 33-33-14.8-33-33-33zm0 6.1c14.8 0 26.8 12 26.8 26.8s-12 26.8-26.8 26.8-26.8-12-26.8-26.8 12-26.8 26.8-26.8zm-114.8 66.7c-10.2 .1-21.6 .4-30.5 1.7 .4 4.4 1.5 18.6 7.1 29.8 9.1-2.6 18.4-3.9 27.6-3.9 41.3 .9 71.5 34.4 78.3 74.5l.1 4.7c10.4 1.9 21.2 2.9 32.2 2.9 11 0 21.8-1 32.2-2.9l.1-4.7c6.8-40.1 37-73.5 78.3-74.5 9.3 0 18.5 1.3 27.6 3.9 5.6-11.1 6.7-25.3 7.1-29.8-8.9-1.3-20.3-1.6-30.5-1.7-18.8 .4-35.2 4.2-48.6 9.7-12.5 16-29.2 30-49.6 33.1-.1 0-.2 0-.3 .1-.1 0-.1 0-.2 .1-5.2 1.1-10.6 1.6-16.2 1.6-5.6 0-11-.5-16.2-1.6-.1 0-.1 0-.2-.1-.1 0-.2 0-.3-.1-20.4-3-37-17-49.6-33.1-13.4-5.5-29.9-9.3-48.6-9.7z" />
                            </svg></a>
                        <div class="icono-texto">Inicio</div>
                    </div>
                </div>
                <br>

                <h2 id="azul">Sable de Luz Azul</h2>
                <p class="republica2"> Habilidad. Usado por excelentes duelistas y personajes muy diestros en los
                    estilos de combate. Durante un combate, prefieren hacer uso de sus habilidades físicas
                    excepcionales más que sus habilidades de la Fuerza. En la Orden Jedi estos eran denominados como
                    guardianes Jedi, y casi siempre se encontraban en misiones, algunos personajes como el Conde
                    Dooku, Obi-Wan Kenobi, Anakin Skywalker, Leia Organa, Barriss Offee, Plo Koon, Ki-Adi-Mundi,
                    Luke Skywalker y Rey llegaron a poseer este color de sable de luz, por otro lado el Stormtrooper
                    desertor, Finn también usó una vez este color de sable para defenderse, aunque no es Jedi como
                    tal.
                </p>

                <div class="galeriasable">
                    <div class="fotosable"><a href="../img/sable/leia-sable-laser.jpg" target="_blank"><img
                                class="fotosable-1" src="../img/sable/leia-sable-laser.jpg" alt="leia"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/obi.jpg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/obi.jpg" alt="obi"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/luke.webp" target="_blank"><img class="fotosable-1"
                                src="../img/sable/luke.webp" alt="luke"></a>
                    </div>
                </div>

                <p class="republica2"><Strong>Empuñadura:</Strong>
                    Suele ser más robusta y funcional, diseñada para el combate.
                    Muchos Jedi como Obi-Wan y Anakin tenían empuñaduras que eran uniformes pero elegantes, con una
                    estética de guerrero.
                    <br>
                    <Strong>Cristal:</Strong>
                    Al igual que con los sables verdes, se utilizan cristales kyber.
                    El cristal azul es el resultado de un proceso natural de afinación y meditación.
                    <br>
                    <Strong>Hoja:</Strong>
                    La hoja azul es intensa y representa la verdad y la lucha por la justicia.
                    Es un símbolo del papel del Jedi como guerrero y defensor.
                </p>

                <!-- Inicio de Pagina -->
                <div class="botones-fin2">
                    <div class="icono-container">

                        <a href="#"><svg xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 512 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                                <path fill="#FFFFFF"
                                    d="M249.9 33.5v26.1C236.3 80.2 226 168.1 225.4 274.9c11.7-15.6 19.1-33.3 19.1-48.2v-16.9c0-5.3 .8-10.5 2.2-15.7 .7-2.1 1.4-4.1 2.6-5.8 1.2-1.8 3.4-3.8 6.7-3.8 3.2 0 5.5 2.1 6.7 3.8 1.2 1.8 2 3.7 2.6 5.8 1.4 5.1 2.2 10.3 2.2 15.7v16.9c0 14.9 7.4 32.6 19.1 48.2-.6-106.8-10.9-194.7-24.5-215.4V33.5h-12.3zm-26.3 147.8c-9.5 2.2-18.7 5.2-27.5 9.1 8.9 16.1 9.8 32.6 1.7 37.3-8 4.6-21.9-4.2-31.4-19.8-11.6 8.8-21.9 19.3-30.6 31.1 14.7 9.6 22.9 22.9 18.3 30.7-4.5 7.7-20 7.1-35.5-1-5.8 13.3-9.8 27.5-11.7 42.4 9.7 .2 18.7 2.4 26.2 6 17.8-.3 32.8-2 40.5-4.2 5.6-26.4 23-48.2 46.3-59.5 .7-25.6 1.9-49.7 3.5-72.1zm65 0c1.6 22.4 2.8 46.5 3.5 72.1 23.3 11.3 40.8 33.2 46.3 59.5 7.7 2.3 22.7 3.9 40.5 4.2 7.5-3.7 16.5-5.9 26.2-6-1.9-14.9-5.9-29.2-11.7-42.4-15.4 8.1-30.9 8.7-35.5 1-4.6-7.7 3.6-21.1 18.3-30.7-8.7-11.8-19-22.3-30.6-31.1-9.5 15.6-23.4 24.4-31.4 19.8-8.1-4.7-7.2-21.2 1.7-37.3a147.5 147.5 0 0 0 -27.5-9.1zm-32.5 8.6c-3.2 0-5.9 8.8-6.1 19.9h-.1v16.9c0 41.4-49 95-93.5 95-52 0-122.8-1.5-156.4 29.2v2.5c9.4 17.1 20.6 33.2 33.2 48C45.7 380.3 84.8 360.4 141.2 360c45.7 1 79 20.3 90.8 40.9 0 0 0 0 0 .1 7.7 2.1 15.9 3.2 24 3.2 8.2 0 16.4-1.1 24-3.2 0 0 0 0 0-.1 11.7-20.5 45.1-39.9 90.8-40.9 56.4 .4 95.5 20.3 108 41.4 12.6-14.8 23.8-30.9 33.2-48v-2.5c-33.6-30.6-104.4-29.2-156.4-29.2-44.5 0-93.5-53.6-93.5-95v-16.9h-.1c-.2-11.1-2.9-19.9-6.1-19.9zm0 96.6c22.4 0 40.6 18.2 40.6 40.6s-18.2 40.7-40.6 40.7-40.6-18.2-40.6-40.7c0-22.4 18.2-40.6 40.6-40.6zm0 7.6c-18.2 0-33 14.8-33 33S237.8 360 256 360s33-14.8 33-33-14.8-33-33-33zm0 6.1c14.8 0 26.8 12 26.8 26.8s-12 26.8-26.8 26.8-26.8-12-26.8-26.8 12-26.8 26.8-26.8zm-114.8 66.7c-10.2 .1-21.6 .4-30.5 1.7 .4 4.4 1.5 18.6 7.1 29.8 9.1-2.6 18.4-3.9 27.6-3.9 41.3 .9 71.5 34.4 78.3 74.5l.1 4.7c10.4 1.9 21.2 2.9 32.2 2.9 11 0 21.8-1 32.2-2.9l.1-4.7c6.8-40.1 37-73.5 78.3-74.5 9.3 0 18.5 1.3 27.6 3.9 5.6-11.1 6.7-25.3 7.1-29.8-8.9-1.3-20.3-1.6-30.5-1.7-18.8 .4-35.2 4.2-48.6 9.7-12.5 16-29.2 30-49.6 33.1-.1 0-.2 0-.3 .1-.1 0-.1 0-.2 .1-5.2 1.1-10.6 1.6-16.2 1.6-5.6 0-11-.5-16.2-1.6-.1 0-.1 0-.2-.1-.1 0-.2 0-.3-.1-20.4-3-37-17-49.6-33.1-13.4-5.5-29.9-9.3-48.6-9.7z" />
                            </svg></a>
                        <div class="icono-texto">Inicio</div>
                    </div>
                </div>
                <br>

                <h2 id="verde">Sable de Luz Verde</h2>
                <p class="republica2"> Sabiduría. Usado por aquellos personajes que son muy poderosos en cuanto a los
                    poderes de La Fuerza. Estos son seres estudiosos e interesados en los misterios de la Fuerza.
                    Durante un combate, prefieren usar la Fuerza en vez del sable. En la Orden Jedi, se les denomina
                    cónsules Jedi, y no salían a misiones muy seguido, sino que se quedaban en el templo Jedi
                    estudiando. Personajes destacados como Qui-Gon Jinn, Yoda, Kit Fisto, Luminara Unduli, Ashoka Tano y
                    Luke Skywalker llegaron a poseer este color de sable.
                </p>

                <div class="galeriasable">
                    <div class="fotosable"><a href="../img/sable/yoda.jpg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/yoda.jpg" alt="yoda"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/fisto.jpg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/fisto.jpg" alt="fisto"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/luke2.jpg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/luke2.jpg" alt="luke2"></a>
                    </div>
                </div>

                <p class="republica2"><Strong>Empuñadura:</Strong>
                    Generalmente estilizada y a menudo hecha de metales duraderos como el aluminio o el acero.
                    Puede incluir detalles ornaméntales que representan la conexión del Jedi con la Fuerza.
                    Los diseños varían entre los Jedi, con ejemplos como la empuñadura de Yoda que es compacta y
                    práctica.
                    <br>
                    <Strong>Cristal:</Strong>
                    El cristal "kyber" es el componente clave que determina el color de la hoja.
                    Los cristales verdes emanan una energía que simboliza la conexión con la Fuerza y están típicamente
                    asociados con la meditación.
                    <br>
                    <Strong>Hoja:</Strong>
                    La hoja del sable verde es brillante y, a menudo, más "viva" que los otros colores.
                    Representa el equilibrio entre la luz y la oscuridad y la habilidad del Jedi para usar la Fuerza en
                    armonía.
                </p>

                <!-- Inicio de Pagina -->
                <div class="botones-fin2">
                    <div class="icono-container">

                        <a href="#"><svg xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 512 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                                <path fill="#FFFFFF"
                                    d="M249.9 33.5v26.1C236.3 80.2 226 168.1 225.4 274.9c11.7-15.6 19.1-33.3 19.1-48.2v-16.9c0-5.3 .8-10.5 2.2-15.7 .7-2.1 1.4-4.1 2.6-5.8 1.2-1.8 3.4-3.8 6.7-3.8 3.2 0 5.5 2.1 6.7 3.8 1.2 1.8 2 3.7 2.6 5.8 1.4 5.1 2.2 10.3 2.2 15.7v16.9c0 14.9 7.4 32.6 19.1 48.2-.6-106.8-10.9-194.7-24.5-215.4V33.5h-12.3zm-26.3 147.8c-9.5 2.2-18.7 5.2-27.5 9.1 8.9 16.1 9.8 32.6 1.7 37.3-8 4.6-21.9-4.2-31.4-19.8-11.6 8.8-21.9 19.3-30.6 31.1 14.7 9.6 22.9 22.9 18.3 30.7-4.5 7.7-20 7.1-35.5-1-5.8 13.3-9.8 27.5-11.7 42.4 9.7 .2 18.7 2.4 26.2 6 17.8-.3 32.8-2 40.5-4.2 5.6-26.4 23-48.2 46.3-59.5 .7-25.6 1.9-49.7 3.5-72.1zm65 0c1.6 22.4 2.8 46.5 3.5 72.1 23.3 11.3 40.8 33.2 46.3 59.5 7.7 2.3 22.7 3.9 40.5 4.2 7.5-3.7 16.5-5.9 26.2-6-1.9-14.9-5.9-29.2-11.7-42.4-15.4 8.1-30.9 8.7-35.5 1-4.6-7.7 3.6-21.1 18.3-30.7-8.7-11.8-19-22.3-30.6-31.1-9.5 15.6-23.4 24.4-31.4 19.8-8.1-4.7-7.2-21.2 1.7-37.3a147.5 147.5 0 0 0 -27.5-9.1zm-32.5 8.6c-3.2 0-5.9 8.8-6.1 19.9h-.1v16.9c0 41.4-49 95-93.5 95-52 0-122.8-1.5-156.4 29.2v2.5c9.4 17.1 20.6 33.2 33.2 48C45.7 380.3 84.8 360.4 141.2 360c45.7 1 79 20.3 90.8 40.9 0 0 0 0 0 .1 7.7 2.1 15.9 3.2 24 3.2 8.2 0 16.4-1.1 24-3.2 0 0 0 0 0-.1 11.7-20.5 45.1-39.9 90.8-40.9 56.4 .4 95.5 20.3 108 41.4 12.6-14.8 23.8-30.9 33.2-48v-2.5c-33.6-30.6-104.4-29.2-156.4-29.2-44.5 0-93.5-53.6-93.5-95v-16.9h-.1c-.2-11.1-2.9-19.9-6.1-19.9zm0 96.6c22.4 0 40.6 18.2 40.6 40.6s-18.2 40.7-40.6 40.7-40.6-18.2-40.6-40.7c0-22.4 18.2-40.6 40.6-40.6zm0 7.6c-18.2 0-33 14.8-33 33S237.8 360 256 360s33-14.8 33-33-14.8-33-33-33zm0 6.1c14.8 0 26.8 12 26.8 26.8s-12 26.8-26.8 26.8-26.8-12-26.8-26.8 12-26.8 26.8-26.8zm-114.8 66.7c-10.2 .1-21.6 .4-30.5 1.7 .4 4.4 1.5 18.6 7.1 29.8 9.1-2.6 18.4-3.9 27.6-3.9 41.3 .9 71.5 34.4 78.3 74.5l.1 4.7c10.4 1.9 21.2 2.9 32.2 2.9 11 0 21.8-1 32.2-2.9l.1-4.7c6.8-40.1 37-73.5 78.3-74.5 9.3 0 18.5 1.3 27.6 3.9 5.6-11.1 6.7-25.3 7.1-29.8-8.9-1.3-20.3-1.6-30.5-1.7-18.8 .4-35.2 4.2-48.6 9.7-12.5 16-29.2 30-49.6 33.1-.1 0-.2 0-.3 .1-.1 0-.1 0-.2 .1-5.2 1.1-10.6 1.6-16.2 1.6-5.6 0-11-.5-16.2-1.6-.1 0-.1 0-.2-.1-.1 0-.2 0-.3-.1-20.4-3-37-17-49.6-33.1-13.4-5.5-29.9-9.3-48.6-9.7z" />
                            </svg></a>
                        <div class="icono-texto">Inicio</div>
                    </div>
                </div>
                <br>
                <h2 id="morada">Sable de Luz Morada</h2>
                <p class="republica2"> El sable de luz morado es un arma icónica dentro del universo de "Star Wars", más
                    conocido por estar asociado a el personaje Mace Windu, interpretado por Samuel L. Jackson en las
                    películas. Este sable de luz se distingue por su color morado brillante, lo que lo hace único entre
                    otros sables de luz, generalmente rojos, azules o verdes.
                </p>
                <p class="republica2"><strong>Fuerza de la Luz y de la Oscuridad:</strong> El color morado a menudo
                    simboliza un
                    equilibrio entre la luz y la oscuridad. Para algunos, representa que su portador está bien entrenado
                    en ambas formas de la Fuerza.
                    <br>
                    <strong>Poder y Honor:</strong> El sable morado de Mace Windu refleja su posición como un maestro
                    Jedi poderoso y un guerrero honorable.
                    <br>
                    <strong>Identidad Única: </strong>Mace Windu fue creado como un personaje destacado, y el sable
                    morado lo diferencia de otros Jedi, acentuando su personalidad y habilidades especiales.
                    <br>
                    <strong>Estilo de Combate:</strong> Mace Windu es conocido por su estilo de combate llamado
                    "Vaapad", que es un enfoque agresivo y acrobático, haciéndolo un adversario formidable en combate.
                </p>

                <div class="galeriasable">
                    <div class="fotosable"><a href="../img/sable/ki-adi-mundi.jpg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/ki-adi-mundi.jpg" alt="ki-adi-mundi"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/mace.jpg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/mace.jpg" alt="mace"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/darhtraya.jpg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/darhtraya.jpg" alt="darhtraya"></a>
                    </div>
                </div>
                <p class="republica2"><Strong>Empuñadura:</Strong>
                    La empuñadura del sable de luz morado de Mace Windu es elegante, con detalles y a menudo diseñada
                    para un agarre cómodo.
                    Mace Windu diseñó su sable para reflejar su estilo personal y su poder como maestro Jedi.
                    <br>
                    <Strong>Cristal:</Strong>
                    El cristal que produce la luz morada es único y, aunque no se detalla ampliamente en las historias,
                    se asocia con el equilibrio entre la luz y la oscuridad.
                    Representa la maestría en el uso tanto de la Fuerza luminosa como de la oscura.
                    <br>
                    <Strong>Hoja:</Strong>
                    La hoja morada es distintiva y representa un enfoque equilibrado del uso de la Fuerza, sugiriendo
                    que su portador puede navegar entre ambos lados.
                    Mace Windu es conocido por su estilo de combate Vaapad, que utiliza la agresión de sus oponentes
                    contra ellos mismos.
                </p>

                <!-- Inicio de Pagina -->
                <div class="botones-fin2">
                    <div class="icono-container">

                        <a href="#"><svg xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 512 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                                <path fill="#FFFFFF"
                                    d="M249.9 33.5v26.1C236.3 80.2 226 168.1 225.4 274.9c11.7-15.6 19.1-33.3 19.1-48.2v-16.9c0-5.3 .8-10.5 2.2-15.7 .7-2.1 1.4-4.1 2.6-5.8 1.2-1.8 3.4-3.8 6.7-3.8 3.2 0 5.5 2.1 6.7 3.8 1.2 1.8 2 3.7 2.6 5.8 1.4 5.1 2.2 10.3 2.2 15.7v16.9c0 14.9 7.4 32.6 19.1 48.2-.6-106.8-10.9-194.7-24.5-215.4V33.5h-12.3zm-26.3 147.8c-9.5 2.2-18.7 5.2-27.5 9.1 8.9 16.1 9.8 32.6 1.7 37.3-8 4.6-21.9-4.2-31.4-19.8-11.6 8.8-21.9 19.3-30.6 31.1 14.7 9.6 22.9 22.9 18.3 30.7-4.5 7.7-20 7.1-35.5-1-5.8 13.3-9.8 27.5-11.7 42.4 9.7 .2 18.7 2.4 26.2 6 17.8-.3 32.8-2 40.5-4.2 5.6-26.4 23-48.2 46.3-59.5 .7-25.6 1.9-49.7 3.5-72.1zm65 0c1.6 22.4 2.8 46.5 3.5 72.1 23.3 11.3 40.8 33.2 46.3 59.5 7.7 2.3 22.7 3.9 40.5 4.2 7.5-3.7 16.5-5.9 26.2-6-1.9-14.9-5.9-29.2-11.7-42.4-15.4 8.1-30.9 8.7-35.5 1-4.6-7.7 3.6-21.1 18.3-30.7-8.7-11.8-19-22.3-30.6-31.1-9.5 15.6-23.4 24.4-31.4 19.8-8.1-4.7-7.2-21.2 1.7-37.3a147.5 147.5 0 0 0 -27.5-9.1zm-32.5 8.6c-3.2 0-5.9 8.8-6.1 19.9h-.1v16.9c0 41.4-49 95-93.5 95-52 0-122.8-1.5-156.4 29.2v2.5c9.4 17.1 20.6 33.2 33.2 48C45.7 380.3 84.8 360.4 141.2 360c45.7 1 79 20.3 90.8 40.9 0 0 0 0 0 .1 7.7 2.1 15.9 3.2 24 3.2 8.2 0 16.4-1.1 24-3.2 0 0 0 0 0-.1 11.7-20.5 45.1-39.9 90.8-40.9 56.4 .4 95.5 20.3 108 41.4 12.6-14.8 23.8-30.9 33.2-48v-2.5c-33.6-30.6-104.4-29.2-156.4-29.2-44.5 0-93.5-53.6-93.5-95v-16.9h-.1c-.2-11.1-2.9-19.9-6.1-19.9zm0 96.6c22.4 0 40.6 18.2 40.6 40.6s-18.2 40.7-40.6 40.7-40.6-18.2-40.6-40.7c0-22.4 18.2-40.6 40.6-40.6zm0 7.6c-18.2 0-33 14.8-33 33S237.8 360 256 360s33-14.8 33-33-14.8-33-33-33zm0 6.1c14.8 0 26.8 12 26.8 26.8s-12 26.8-26.8 26.8-26.8-12-26.8-26.8 12-26.8 26.8-26.8zm-114.8 66.7c-10.2 .1-21.6 .4-30.5 1.7 .4 4.4 1.5 18.6 7.1 29.8 9.1-2.6 18.4-3.9 27.6-3.9 41.3 .9 71.5 34.4 78.3 74.5l.1 4.7c10.4 1.9 21.2 2.9 32.2 2.9 11 0 21.8-1 32.2-2.9l.1-4.7c6.8-40.1 37-73.5 78.3-74.5 9.3 0 18.5 1.3 27.6 3.9 5.6-11.1 6.7-25.3 7.1-29.8-8.9-1.3-20.3-1.6-30.5-1.7-18.8 .4-35.2 4.2-48.6 9.7-12.5 16-29.2 30-49.6 33.1-.1 0-.2 0-.3 .1-.1 0-.1 0-.2 .1-5.2 1.1-10.6 1.6-16.2 1.6-5.6 0-11-.5-16.2-1.6-.1 0-.1 0-.2-.1-.1 0-.2 0-.3-.1-20.4-3-37-17-49.6-33.1-13.4-5.5-29.9-9.3-48.6-9.7z" />
                            </svg></a>
                        <div class="icono-texto">Inicio</div>
                    </div>
                </div>
                <br>
                <h2 id="amarilla">Sable de Luz Amarilla</h2>

                <p class="republica2"><strong> Personaje Asociado:</strong> Ahsoka Tano es la figura más reconocida que
                    empuña sables de luz amarillos. Ella es una ex-Jedi que aparece en Star Wars: The Clone Wars, Star
                    Wars Rebels y The Mandalorian.
                    <br>
                    <strong>Simbolismo:</strong> Los sables amarillos son menos comunes que los colores azul y verde
                    (que son típicos de los Jedi) y el rojo (que caracteriza a los Sith). El amarillo se asocia con un
                    enfoque más equilibrado y una perspectiva que no es estrictamente Jedi o Sith. Representa la idea de
                    un guardián o un protector sin estar atado a las normas tradicionales de la Orden Jedi.
                    <br>
                    <strong>Uso en la Historia: </strong>Ahsoka utiliza estos sables de luz en su búsqueda de la
                    justicia y la lucha contra el lado oscuro, después de separarse de la Orden Jedi. Su elección de
                    color simboliza su camino individual y su deseo de seguir su propio camino moral.
                    <br>
                </p>

                <div class="galeriasable">
                    <div class="fotosable"><a href="../img/sable/ahsoka.jpg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/ahsoka.jpg" alt="Ahsoka"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/rey.jpg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/rey.jpg" alt="rey"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/sableamarillo.jpg" target="_blank"><img
                                class="fotosable-1" src="../img/sable/sableamarillo.jpg" alt="sableamarillo"></a>
                    </div>
                </div>
                <p class="republica2"><Strong>Empuñadura:</Strong>
                    La empuñadura de los sables de luz amarillos de Ahsoka Tano es distintiva, diseñada para ajustarse a
                    su estilo de combate acrobático y ágil. Generalmente, su forma es más elegante y estilizada que las
                    empuñaduras tradicionales de otros Jedi.
                    La empuñadura tiene un acabado metálico con detalles que reflejan su conexión con la cultura Jedi, a
                    la vez que mantiene un aspecto único.
                    <br>
                    <Strong>Cristal:</Strong>
                    El cristal del sable de luz amarillo es un cristal kyber que Ahsoka obtuvo de una manera que no se
                    ha detallado en profundidad en las historias canónicas.
                    A diferencia de los antiguos Jedi que imbuían sus sables a partir de la tradición, Ahsoka, siendo
                    una ex-Jedi, simboliza un enfoque más personalizado y menos ortodoxo hacia la Fuerza.
                    <br>
                    <Strong>Hoja:</Strong>
                    La hoja del sable de luz amarillo es de un brillante color amarillo dorado, que simboliza su
                    naturaleza equilibrada entre el camino Jedi y su independencia.
                    La intensidad del color puede cambiar ligeramente dependiendo de la iluminación y el contexto,
                    reflejando su conexión tanto con la Luz como con la sombra.
                </p>

                <!-- Inicio de Pagina -->
                <div class="botones-fin2">
                    <div class="icono-container">

                        <a href="#"><svg xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 512 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                                <path fill="#FFFFFF"
                                    d="M249.9 33.5v26.1C236.3 80.2 226 168.1 225.4 274.9c11.7-15.6 19.1-33.3 19.1-48.2v-16.9c0-5.3 .8-10.5 2.2-15.7 .7-2.1 1.4-4.1 2.6-5.8 1.2-1.8 3.4-3.8 6.7-3.8 3.2 0 5.5 2.1 6.7 3.8 1.2 1.8 2 3.7 2.6 5.8 1.4 5.1 2.2 10.3 2.2 15.7v16.9c0 14.9 7.4 32.6 19.1 48.2-.6-106.8-10.9-194.7-24.5-215.4V33.5h-12.3zm-26.3 147.8c-9.5 2.2-18.7 5.2-27.5 9.1 8.9 16.1 9.8 32.6 1.7 37.3-8 4.6-21.9-4.2-31.4-19.8-11.6 8.8-21.9 19.3-30.6 31.1 14.7 9.6 22.9 22.9 18.3 30.7-4.5 7.7-20 7.1-35.5-1-5.8 13.3-9.8 27.5-11.7 42.4 9.7 .2 18.7 2.4 26.2 6 17.8-.3 32.8-2 40.5-4.2 5.6-26.4 23-48.2 46.3-59.5 .7-25.6 1.9-49.7 3.5-72.1zm65 0c1.6 22.4 2.8 46.5 3.5 72.1 23.3 11.3 40.8 33.2 46.3 59.5 7.7 2.3 22.7 3.9 40.5 4.2 7.5-3.7 16.5-5.9 26.2-6-1.9-14.9-5.9-29.2-11.7-42.4-15.4 8.1-30.9 8.7-35.5 1-4.6-7.7 3.6-21.1 18.3-30.7-8.7-11.8-19-22.3-30.6-31.1-9.5 15.6-23.4 24.4-31.4 19.8-8.1-4.7-7.2-21.2 1.7-37.3a147.5 147.5 0 0 0 -27.5-9.1zm-32.5 8.6c-3.2 0-5.9 8.8-6.1 19.9h-.1v16.9c0 41.4-49 95-93.5 95-52 0-122.8-1.5-156.4 29.2v2.5c9.4 17.1 20.6 33.2 33.2 48C45.7 380.3 84.8 360.4 141.2 360c45.7 1 79 20.3 90.8 40.9 0 0 0 0 0 .1 7.7 2.1 15.9 3.2 24 3.2 8.2 0 16.4-1.1 24-3.2 0 0 0 0 0-.1 11.7-20.5 45.1-39.9 90.8-40.9 56.4 .4 95.5 20.3 108 41.4 12.6-14.8 23.8-30.9 33.2-48v-2.5c-33.6-30.6-104.4-29.2-156.4-29.2-44.5 0-93.5-53.6-93.5-95v-16.9h-.1c-.2-11.1-2.9-19.9-6.1-19.9zm0 96.6c22.4 0 40.6 18.2 40.6 40.6s-18.2 40.7-40.6 40.7-40.6-18.2-40.6-40.7c0-22.4 18.2-40.6 40.6-40.6zm0 7.6c-18.2 0-33 14.8-33 33S237.8 360 256 360s33-14.8 33-33-14.8-33-33-33zm0 6.1c14.8 0 26.8 12 26.8 26.8s-12 26.8-26.8 26.8-26.8-12-26.8-26.8 12-26.8 26.8-26.8zm-114.8 66.7c-10.2 .1-21.6 .4-30.5 1.7 .4 4.4 1.5 18.6 7.1 29.8 9.1-2.6 18.4-3.9 27.6-3.9 41.3 .9 71.5 34.4 78.3 74.5l.1 4.7c10.4 1.9 21.2 2.9 32.2 2.9 11 0 21.8-1 32.2-2.9l.1-4.7c6.8-40.1 37-73.5 78.3-74.5 9.3 0 18.5 1.3 27.6 3.9 5.6-11.1 6.7-25.3 7.1-29.8-8.9-1.3-20.3-1.6-30.5-1.7-18.8 .4-35.2 4.2-48.6 9.7-12.5 16-29.2 30-49.6 33.1-.1 0-.2 0-.3 .1-.1 0-.1 0-.2 .1-5.2 1.1-10.6 1.6-16.2 1.6-5.6 0-11-.5-16.2-1.6-.1 0-.1 0-.2-.1-.1 0-.2 0-.3-.1-20.4-3-37-17-49.6-33.1-13.4-5.5-29.9-9.3-48.6-9.7z" />
                            </svg></a>
                        <div class="icono-texto">Inicio</div>
                    </div>
                </div>
                <br>
                <h2 id="negro">Sable de Luz Negro (Darksaber)</h2>

                <p class="republica2"><strong> Origen: </strong> El Darksaber fue creado por Tarre Vizsla, el primer
                    Mandaloriano que fue aceptado en la Orden Jedi. Se forjó durante la época de la Antigua República y
                    tiene un diseño diferente al de los sables de luz tradicionales, con una hoja plana y negra.

                    <br>
                    <strong>Simbolismo: </strong> El Darksaber es un símbolo de liderazgo entre los Mandalorianos. Se
                    cree que quien empuñe el Darksaber y lo derrote en combate tiene el derecho a gobernar Mandalore.
                    Esto le otorga un gran peso y relevancia cultural dentro de la sociedad Mandaloriana.
                    <br>
                    <strong>Importancia en la Narrativa: </strong> El Darksaber se ha presentado a lo largo de diversas
                    series de Star Wars, incluyendo Star Wars: The Clone Wars y Star Wars Rebels. En The Mandalorian, el
                    sable es reclamado por Din Djarin (Mando) tras derrotar a Moff Gideon, lo que causa tensiones, ya
                    que según las tradiciones Mandalorianas, esto implica que ahora tiene derecho a gobernar. El
                    Darksaber simboliza la unidad y el conflicto dentro de la cultura Mandaloriana.
                    <br>
                </p>

                <div class="galeriasable">
                    <div class="fotosable"><a href="../img/sable/tarrevizsla.jpeg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/tarrevizsla.jpeg" alt="tarrevizsla"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/dindjarin.jpg" target="_blank"><img class="fotosable-1"
                                src="../img/sable/dindjarin.jpg" alt="dindjarin"></a>
                    </div>

                    <div class="fotosable"><a href="../img/sable/maul.webp" target="_blank"><img
                                class="fotosable-1" src="../img/sable/maul.webp" alt="maul"></a>
                    </div>
                </div>
                <p class="republica2"><Strong>Empuñadura:</Strong>
                    El Darksaber tiene una empuñadura más ancha y robusta en comparación con los sables de luz
                    tradicionales, diseñada para su uso en combates cuerpo a cuerpo durante la guerra.
                    La empuñadura es negra con detalles que a menudo tienen aspectos Mandalorianos, marcando su
                    importancia en la cultura de este pueblo.
                    <br>
                    <Strong>Cristal:</Strong>
                    El Darksaber es único en su forma y puede tener su propio conjunto de cristales kyber que logran
                    crear la distintiva hoja negra.
                    Sin embargo, el lore no se ha profundizado exhaustivamente sobre el cristal específico utilizado, lo
                    que hace que su naturaleza exacta siga siendo enigmática. A lo largo de la historia, el Darksaber se
                    ha pasado de un guerrero Mandaloriano a otro, indicando que tiene un poderoso significado más allá
                    de su material.
                    <br>
                    <Strong>Hoja:</Strong>
                    La hoja del Darksaber es de un color negro sólido, con bordes que a menudo resplandecen de manera
                    más intensa. Esto la distingue claramente de cualquier otra hoja de sable de luz.
                    El diseño parece más plano que las hojas tradicionales, lo que le da una apariencia distintiva y
                    amenazadora. Además, su presencia se siente más poderosa e intimidante, reflejando su historia de
                    lucha y liderazgo.
                </p>








            </main>
        </div>

        <div class="articulo6">
            <div class="botones-fin">
                <!-- Inicio de Pagina -->
                <div class="icono-container">

                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 512 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                            <path fill="#FFFFFF"
                                d="M249.9 33.5v26.1C236.3 80.2 226 168.1 225.4 274.9c11.7-15.6 19.1-33.3 19.1-48.2v-16.9c0-5.3 .8-10.5 2.2-15.7 .7-2.1 1.4-4.1 2.6-5.8 1.2-1.8 3.4-3.8 6.7-3.8 3.2 0 5.5 2.1 6.7 3.8 1.2 1.8 2 3.7 2.6 5.8 1.4 5.1 2.2 10.3 2.2 15.7v16.9c0 14.9 7.4 32.6 19.1 48.2-.6-106.8-10.9-194.7-24.5-215.4V33.5h-12.3zm-26.3 147.8c-9.5 2.2-18.7 5.2-27.5 9.1 8.9 16.1 9.8 32.6 1.7 37.3-8 4.6-21.9-4.2-31.4-19.8-11.6 8.8-21.9 19.3-30.6 31.1 14.7 9.6 22.9 22.9 18.3 30.7-4.5 7.7-20 7.1-35.5-1-5.8 13.3-9.8 27.5-11.7 42.4 9.7 .2 18.7 2.4 26.2 6 17.8-.3 32.8-2 40.5-4.2 5.6-26.4 23-48.2 46.3-59.5 .7-25.6 1.9-49.7 3.5-72.1zm65 0c1.6 22.4 2.8 46.5 3.5 72.1 23.3 11.3 40.8 33.2 46.3 59.5 7.7 2.3 22.7 3.9 40.5 4.2 7.5-3.7 16.5-5.9 26.2-6-1.9-14.9-5.9-29.2-11.7-42.4-15.4 8.1-30.9 8.7-35.5 1-4.6-7.7 3.6-21.1 18.3-30.7-8.7-11.8-19-22.3-30.6-31.1-9.5 15.6-23.4 24.4-31.4 19.8-8.1-4.7-7.2-21.2 1.7-37.3a147.5 147.5 0 0 0 -27.5-9.1zm-32.5 8.6c-3.2 0-5.9 8.8-6.1 19.9h-.1v16.9c0 41.4-49 95-93.5 95-52 0-122.8-1.5-156.4 29.2v2.5c9.4 17.1 20.6 33.2 33.2 48C45.7 380.3 84.8 360.4 141.2 360c45.7 1 79 20.3 90.8 40.9 0 0 0 0 0 .1 7.7 2.1 15.9 3.2 24 3.2 8.2 0 16.4-1.1 24-3.2 0 0 0 0 0-.1 11.7-20.5 45.1-39.9 90.8-40.9 56.4 .4 95.5 20.3 108 41.4 12.6-14.8 23.8-30.9 33.2-48v-2.5c-33.6-30.6-104.4-29.2-156.4-29.2-44.5 0-93.5-53.6-93.5-95v-16.9h-.1c-.2-11.1-2.9-19.9-6.1-19.9zm0 96.6c22.4 0 40.6 18.2 40.6 40.6s-18.2 40.7-40.6 40.7-40.6-18.2-40.6-40.7c0-22.4 18.2-40.6 40.6-40.6zm0 7.6c-18.2 0-33 14.8-33 33S237.8 360 256 360s33-14.8 33-33-14.8-33-33-33zm0 6.1c14.8 0 26.8 12 26.8 26.8s-12 26.8-26.8 26.8-26.8-12-26.8-26.8 12-26.8 26.8-26.8zm-114.8 66.7c-10.2 .1-21.6 .4-30.5 1.7 .4 4.4 1.5 18.6 7.1 29.8 9.1-2.6 18.4-3.9 27.6-3.9 41.3 .9 71.5 34.4 78.3 74.5l.1 4.7c10.4 1.9 21.2 2.9 32.2 2.9 11 0 21.8-1 32.2-2.9l.1-4.7c6.8-40.1 37-73.5 78.3-74.5 9.3 0 18.5 1.3 27.6 3.9 5.6-11.1 6.7-25.3 7.1-29.8-8.9-1.3-20.3-1.6-30.5-1.7-18.8 .4-35.2 4.2-48.6 9.7-12.5 16-29.2 30-49.6 33.1-.1 0-.2 0-.3 .1-.1 0-.1 0-.2 .1-5.2 1.1-10.6 1.6-16.2 1.6-5.6 0-11-.5-16.2-1.6-.1 0-.1 0-.2-.1-.1 0-.2 0-.3-.1-20.4-3-37-17-49.6-33.1-13.4-5.5-29.9-9.3-48.6-9.7z" />
                        </svg></a>
                    <div class="icono-texto">Inicio</div>

                </div>
                <!--Home -->
                <div class="icono-container">

                    <a href="inicio.php"><svg xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 448 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                            <path fill="#FFFFFF"
                                d="M0 96C0 43 43 0 96 0L384 0l32 0c17.7 0 32 14.3 32 32l0 320c0 17.7-14.3 32-32 32l0 64c17.7 0 32 14.3 32 32s-14.3 32-32 32l-32 0L96 512c-53 0-96-43-96-96L0 96zM64 416c0 17.7 14.3 32 32 32l256 0 0-64L96 384c-17.7 0-32 14.3-32 32zm90.4-234.4l-21.2-21.2c-3 10.1-5.1 20.6-5.1 31.6c0 .2 0 .5 .1 .8s.1 .5 .1 .8L165.2 226c2.5 2.1 3.4 5.8 2.3 8.9c-1.3 3-4.1 5.1-7.5 5.1c-1.9-.1-3.8-.8-5.2-2l-23.6-20.6C142.8 267 186.9 304 240 304s97.3-37 108.9-86.6L325.3 238c-1.4 1.2-3.3 2-5.3 2c-2.2-.1-4.4-1.1-6-2.8c-1.2-1.5-1.9-3.4-2-5.2c.1-2.2 1.1-4.4 2.8-6l37.1-32.5c0-.3 0-.5 .1-.8s.1-.5 .1-.8c0-11-2.1-21.5-5.1-31.6l-21.2 21.2c-3.1 3.1-8.1 3.1-11.3 0s-3.1-8.1 0-11.2l26.4-26.5c-8.2-17-20.5-31.7-35.9-42.6c-2.7-1.9-6.2 1.4-5 4.5c8.5 22.4 3.6 48-13 65.6c-3.2 3.4-3.6 8.9-.9 12.7c9.8 14 12.7 31.9 7.5 48.5c-5.9 19.4-22 34.1-41.9 38.3l-1.4-34.3 12.6 8.6c.6 .4 1.5 .6 2.3 .6c1.5 0 2.7-.8 3.5-2s.6-2.8-.1-4L260 225.4l18-3.6c1.8-.4 3.1-2.1 3.1-4s-1.4-3.5-3.1-3.9l-18-3.7 8.5-14.3c.8-1.2 .9-2.9 .1-4.1s-2-2-3.5-2l-.1 0c-.7 .1-1.5 .3-2.1 .7l-14.1 9.6L244 87.9c-.1-2.2-1.9-3.9-4-3.9s-3.9 1.6-4 3.9l-4.6 110.8-12-8.1c-1.5-1.1-3.6-.9-5 .4s-1.6 3.4-.8 5l8.6 14.3-18 3.7c-1.8 .4-3.1 2-3.1 3.9s1.4 3.6 3.1 4l18 3.8-8.6 14.2c-.2 .6-.5 1.4-.5 2c0 1.1 .5 2.1 1.2 3c.8 .6 1.8 1 2.8 1c.7 0 1.6-.2 2.2-.6l10.4-7.1-1.4 32.8c-19.9-4.1-36-18.9-41.9-38.3c-5.1-16.6-2.2-34.4 7.6-48.5c2.7-3.9 2.3-9.3-.9-12.7c-16.6-17.5-21.6-43.1-13.1-65.5c1.2-3.1-2.3-6.4-5-4.5c-15.3 10.9-27.6 25.6-35.8 42.6l26.4 26.5c3.1 3.1 3.1 8.1 0 11.2s-8.1 3.1-11.2 0z" />
                        </svg></a>
                    <div class="icono-texto">Home</div>

                </div>
                <!-- Pagina anterior -->
                <div class="icono-container">

                    <div class="anterior">
                        <a onClick="history.go(-1)"><svg xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 512 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                                <path fill="#FFFFFF"
                                    d="M249.9 33.5v26.1C236.3 80.2 226 168.1 225.4 274.9c11.7-15.6 19.1-33.3 19.1-48.2v-16.9c0-5.3 .8-10.5 2.2-15.7 .7-2.1 1.4-4.1 2.6-5.8 1.2-1.8 3.4-3.8 6.7-3.8 3.2 0 5.5 2.1 6.7 3.8 1.2 1.8 2 3.7 2.6 5.8 1.4 5.1 2.2 10.3 2.2 15.7v16.9c0 14.9 7.4 32.6 19.1 48.2-.6-106.8-10.9-194.7-24.5-215.4V33.5h-12.3zm-26.3 147.8c-9.5 2.2-18.7 5.2-27.5 9.1 8.9 16.1 9.8 32.6 1.7 37.3-8 4.6-21.9-4.2-31.4-19.8-11.6 8.8-21.9 19.3-30.6 31.1 14.7 9.6 22.9 22.9 18.3 30.7-4.5 7.7-20 7.1-35.5-1-5.8 13.3-9.8 27.5-11.7 42.4 9.7 .2 18.7 2.4 26.2 6 17.8-.3 32.8-2 40.5-4.2 5.6-26.4 23-48.2 46.3-59.5 .7-25.6 1.9-49.7 3.5-72.1zm65 0c1.6 22.4 2.8 46.5 3.5 72.1 23.3 11.3 40.8 33.2 46.3 59.5 7.7 2.3 22.7 3.9 40.5 4.2 7.5-3.7 16.5-5.9 26.2-6-1.9-14.9-5.9-29.2-11.7-42.4-15.4 8.1-30.9 8.7-35.5 1-4.6-7.7 3.6-21.1 18.3-30.7-8.7-11.8-19-22.3-30.6-31.1-9.5 15.6-23.4 24.4-31.4 19.8-8.1-4.7-7.2-21.2 1.7-37.3a147.5 147.5 0 0 0 -27.5-9.1zm-32.5 8.6c-3.2 0-5.9 8.8-6.1 19.9h-.1v16.9c0 41.4-49 95-93.5 95-52 0-122.8-1.5-156.4 29.2v2.5c9.4 17.1 20.6 33.2 33.2 48C45.7 380.3 84.8 360.4 141.2 360c45.7 1 79 20.3 90.8 40.9 0 0 0 0 0 .1 7.7 2.1 15.9 3.2 24 3.2 8.2 0 16.4-1.1 24-3.2 0 0 0 0 0-.1 11.7-20.5 45.1-39.9 90.8-40.9 56.4 .4 95.5 20.3 108 41.4 12.6-14.8 23.8-30.9 33.2-48v-2.5c-33.6-30.6-104.4-29.2-156.4-29.2-44.5 0-93.5-53.6-93.5-95v-16.9h-.1c-.2-11.1-2.9-19.9-6.1-19.9zm0 96.6c22.4 0 40.6 18.2 40.6 40.6s-18.2 40.7-40.6 40.7-40.6-18.2-40.6-40.7c0-22.4 18.2-40.6 40.6-40.6zm0 7.6c-18.2 0-33 14.8-33 33S237.8 360 256 360s33-14.8 33-33-14.8-33-33-33zm0 6.1c14.8 0 26.8 12 26.8 26.8s-12 26.8-26.8 26.8-26.8-12-26.8-26.8 12-26.8 26.8-26.8zm-114.8 66.7c-10.2 .1-21.6 .4-30.5 1.7 .4 4.4 1.5 18.6 7.1 29.8 9.1-2.6 18.4-3.9 27.6-3.9 41.3 .9 71.5 34.4 78.3 74.5l.1 4.7c10.4 1.9 21.2 2.9 32.2 2.9 11 0 21.8-1 32.2-2.9l.1-4.7c6.8-40.1 37-73.5 78.3-74.5 9.3 0 18.5 1.3 27.6 3.9 5.6-11.1 6.7-25.3 7.1-29.8-8.9-1.3-20.3-1.6-30.5-1.7-18.8 .4-35.2 4.2-48.6 9.7-12.5 16-29.2 30-49.6 33.1-.1 0-.2 0-.3 .1-.1 0-.1 0-.2 .1-5.2 1.1-10.6 1.6-16.2 1.6-5.6 0-11-.5-16.2-1.6-.1 0-.1 0-.2-.1-.1 0-.2 0-.3-.1-20.4-3-37-17-49.6-33.1-13.4-5.5-29.9-9.3-48.6-9.7z" />
                            </svg></a>

                    </div>
                    <div class="icono-texto2">Anterior</div>


                </div>
            </div>

        </div>

        <div class="articulo7">
            <aside>

            </aside>
        </div>

        <div class="articulo8">
            <footer>
                <div class="marquee">
                    <div>
                        <span>Todos los derechos reservados - C.F.P Nº20 - Copyright &copy; -2024 - Argentina </span>
                    </div>
                </div>



            </footer>

        </div>
    </div>



    <script src="../js/sables.js"></script>
</body>

</html>